...
WininetHttp::WininetHttp():m_hSession(NULL), m_hConnect(NULL), m_hRequest(NULL)
{
}
...
const std::string WininetHttp::RequestJsonInfo(const std::string& lpUrl,
...
int WininetHttp::ParseJsonInfo(const std::string &strJsonInfo)
{
	Json::Reader reader;                                    //解析json用Json::Reader
	Json::Value value;                                        //可以代表任意类型
	if (!reader.parse(strJsonInfo, value))
	{
		std::string cause = value["message"].asString();  // 访问节点，upload_id = "UP000000"  
		int result = value["result"].asInt();    // 访问节点，code = 100 
		//CVLog.LogMessage(LOG_LEVEL_ERROR, "[CXLDbDataMgr::GetVideoGisData] Video Gis parse data error...");
		return 0;
	}
	if (!value["result"].isNull())
	{
		int a=value["result"].asInt();
		return a;
	}
	/*if (!value["message"].isNull())
	{
		int nSize = value["message"].size();
		for (int nPos = 0; nPos < nSize; ++nPos)                //对数据数组进行遍历
		{
			//std::list<string> lstCardDevData;
			//PGCARDDEVDATA stru ;
			//stru.title        = value["message"][nPos]["title"].asString();            //标题
			//stru.content        = value["message"][nPos]["content"].asString();     //内容
			//stru.takeType     = value["message"][nPos]["takeType"].asString();    //类型
...
// 解析Json数据 [11/8/2016/shike]
string WininetHttp::ParseJsonMess(const std::string &strJsonInfo)
{
	string message;
	Json::Reader reader;                                    //解析json用Json::Reader
	Json::Value value;                                        //可以代表任意类型
...
//http://117.131.227.94:8045/bpmx/perfRemind/perfRemind/perfRemindinfo/getTaskForExe.ht?username=admin&password=123456
void WininetHttp::ParseURLWeb(std::string strUrl, std::string& strHostName, std::string& strPageName, WORD& sPort)
{
	sPort = 80;
...
char* WininetHttp::UtfToGbk(const char* utf8)
{
